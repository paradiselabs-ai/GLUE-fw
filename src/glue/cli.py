...
async def run_app(
    ...
    log_file = os.path.join(LOGS_DIR, f"glue_cli_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
    setup_logging(log_file_path=log_file) # Setup logging to file

    logger = logging.getLogger("glue.cli")
    logger.info(f"GLUE CLI version {__version__} started.")
    logger.info(f"Running app with config: {config_file}, interactive: {interactive}, engine: {engine}")

    if not os.path.exists(config_file):
        display_warning(console, f"Configuration file not found: {config_file}")
    ...
    try:
        ...
        if not parsed_config:
            display_warning(console, f"Failed to parse configuration file: {config_file}")
            logger.error(f"Failed to parse configuration file: {config_file}. Parser returned empty.")
            return False
        logger.debug(f"Successfully parsed config file: {config_file}")

    except Exception as e:
        display_warning(console, f"Error parsing GLUE file '{config_file}': {e}")
        logger.error(f"Error parsing GLUE file '{config_file}': {e}", exc_info=True)
        if CLI_CONFIG.get("display", {}).get("verbose_errors", False):
            console.print_exception()
        return False

    if engine == 'glue':
        logger.info("Using GLUE engine.")
        try:
            app = GlueApp(config=parsed_config) # Pass the parsed_config dictionary directly
            logger.info(f"GLUE App '{app.name}' initialized successfully.")

            if interactive:
                logger.info("Starting interactive session with GLUE engine.")
                await run_interactive_session(app, console=console) # Pass console here
            elif input_text:
                logger.info(f"Processing input text with GLUE engine: {input_text}")
                # Non-interactive direct execution (basic example)
                # This part needs a proper implementation based on how GlueApp handles direct input
                # For now, we'll assume a simple default processing method if available.
                if hasattr(app, 'process_input'):
                    response = await app.process_input(input_text)
                    console.print(f"[bold green]Response:[/] {response}")
                else:
                    console.print("[yellow]GlueApp does not have a 'process_input' method for non-interactive mode.[/]")
                logger.info("Non-interactive GLUE engine run completed.")
            else:
                console.print("[yellow]No input text provided for non-interactive GLUE engine run.[/]")
                logger.warning("No input text for non-interactive GLUE engine run.")
            return True
        except Exception as e:
            display_warning(console, f"Error initializing or running GLUE application: {e}")
            logger.error(f"Error with GLUE engine: {e}", exc_info=True)
            if CLI_CONFIG.get("display", {}).get("verbose_errors", False):
                console.print_exception()
            return False

    elif engine == 'agno':
        logger.info("Using Agno engine.")
        try:
            agno_adapter = GlueAgnoAdapter()
            logger.debug("GlueAgnoAdapter initialized.")

            if interactive:
                logger.info("Attempting to start interactive session with Agno engine via GlueApp.")
                # Use the new method to create a GlueApp instance from config
                glue_app_instance = await agno_adapter.create_glue_app_from_config(parsed_config)
                
                if glue_app_instance:
                    logger.info(f"GlueApp '{glue_app_instance.name}' created for Agno interactive mode. Starting session.")
                    await run_interactive_session(glue_app_instance, console=console) # Pass console here
                else:
                    error_msg = "Failed to create GLUE application for Agno interactive mode. Check logs for details."
                    display_warning(console, error_msg)
                    logger.error(error_msg)
                    return False
            else:
                # Non-interactive Agno run (original path)
                logger.info("Running Agno workflow non-interactively.")
                if input_text:
                    # Agno might not support direct input_text in the same way. 
                    # Its run_workflow_from_file usually handles the entire flow.
                    # We might need to pass input_text to the Agno workflow if it supports it.
                    display_warning(console, "Non-interactive mode with Agno engine typically runs the full .glue workflow. 'input_text' might be ignored unless the workflow is designed to use it.")
                    logger.warning("Input text provided for non-interactive Agno run. Workflow might not use it.")
                
                # This path assumes agno_adapter.run_workflow_from_file uses the original `setup`
                # method of GlueAgnoAdapter which populates dictionaries for Agno.
                run_result = await agno_adapter.run_workflow_from_file(config_file)
                if run_result:
                    console.print(Panel(Markdown(f"**Agno Workflow Result:**\n\n```json\n{json.dumps(run_result, indent=2)}\n```"), title="Agno Output", expand=False, border_style="green"))
                    logger.info(f"Agno non-interactive run completed. Result: {run_result}")
                else:
                    display_warning(console, "Agno workflow execution failed or returned no result.")
                    logger.error("Agno non-interactive run failed or returned no result.")
                    return False
            return True
        except ImportError:
            display_warning(console, "Agno adapter or its dependencies not found. Please ensure Agno is installed.")
            logger.error("Agno adapter import error.", exc_info=True)
            return False
        except Exception as e:
            display_warning(console, f"Error running Agno engine: {e}")
            logger.error(f"Error with Agno engine: {e}", exc_info=True)
            if CLI_CONFIG.get("display", {}).get("verbose_errors", False):
                console.print_exception()
            return False

    else:
        display_warning(console, f"Unsupported engine: {engine}. Choose 'glue' or 'agno'.")
...
# ==================== Logging Setup ====================
def setup_logging(level=logging.INFO, log_file_path=None):
    ...
    logging.basicConfig(
        ...
        handlers=[logging.FileHandler(log_file_path, mode="a") if log_file_path else logging.FileHandler(os.path.join(LOGS_DIR, "glue.log"), mode="a")],
    )
    ...
    for logger_name in ["httpcore", "httpx", "openai", "markdown_it", "rich"]:
        ...
        third_party_logger.addHandler(
            logging.FileHandler(log_file_path if log_file_path else os.path.join(LOGS_DIR, "glue.log"), mode="a")
        )
