    ...
    try:
        ...
        if "app" in config:
            ...
            if "config" in app_details:
                app_config["development"] = app_details["config"].get("development", True)
        ...
        if "magnets" in config and config["magnets"]:
            app_config["magnets"] = [MagnetConfig(**magnet) for magnet in config["magnets"]]
        else:
            app_config["magnets"] = []
...
def validate_config(config: Dict[str, Any]) -> List[str]:
    ...
    try:
        ...
        return str(e).split("\n")[1:]  # Skip the first line which is the general error message
