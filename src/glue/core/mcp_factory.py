...
import asyncio
import re
from typing import Dict, Any, Optional, Type, Union, Callable, Set, List
import logging
from pydantic import BaseModel, Field, ConfigDict

from .mcp import MCPServer, MCPTool, MCPConfig
from ..tools.tool_base import Tool, ToolConfig, ToolPermission
from ..core.types import AdhesiveType
from .sandbox import CodeSandbox, SandboxConfig, SandboxViolation
...
class DynamicMCPFactory:
                ...
                class DynamicSandboxedTool(Tool):
                    def __init__(self, name: str, description: str):
                        config = ToolConfig(
                            ...
                            adhesive_types={AdhesiveType.TAPE, AdhesiveType.VELCRO, AdhesiveType.GLUE}
                        )
                    ...
                    async def _execute(self, parameters: Dict[str, Any]) -> Any:
                        ...
                        if parameters is None:
                            parameters = {}
                        return await self.sandbox.execute_function(self.func, parameters)
    ...
    async def parse_natural_request(
        ...
        server_match = re.search(r"create (?:an? )?mcp (?:server )?(?:that |to )?(.*?)(?:\.|$)", request.lower())
        if not server_match:
