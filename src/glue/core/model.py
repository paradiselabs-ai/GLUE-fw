    ...
    def __init__(self, config=None, **kwargs):
        ...
        if config is None:
            ...
            try:
                ...
                print(
                    f"Warning: Model config is not a dictionary or easily convertible: {type(config)}. Initializing provider might fail."
                )
                config = {}
        ...
        logger.debug(
            f"Model {config.get('name', 'unnamed')}: Raw adhesives from config: {adhesives}"
        )
            ...
            if isinstance(adhesive, str):
                try:
                    ...
                    adhesive_type = next(
                        (
                            at
                            for at in AdhesiveType
                            if at.value.lower() == adhesive.lower()
                        ),
                        None,
                    )
                    if adhesive_type:
                        ...
                        logger.debug(
                            f"Added adhesive {adhesive_type} from string '{adhesive}'"
                        )
                    else:
                        logger.warning(f"Unknown adhesive type: {adhesive}")
                except Exception as e:
                    logger.warning(
                        f"Error converting adhesive '{adhesive}' to enum: {e}"
                    )
            else:
        ...
        logger.info(
            f"Model {config.get('name', 'unnamed')} initialized with adhesives: {self.adhesives}"
        )
