...
class Subtask(BaseModel):
    ...
    dependencies: List[str] = Field(
        default_factory=list, description="Prerequisite subtask IDs"
    )
    assigned_agent_id: Optional[str] = Field(
        None,
        description="Specific agent ID this subtask should ideally be assigned to, if specified.",
    )
...
class TaskRecord(BaseModel):
    ...
    task_id: Optional[str] = Field(
        None, description="Assigned task ID from delegation tool"
    )
    timestamp: Optional[str] = Field(
        None, description="ISO timestamp when task was delegated or updated"
    )
...
class DecomposeOutput(BaseModel):
    ...
    subtasks: List[Subtask] = Field(
        ..., description="List of decomposed subtasks with dependencies"
    )
...
class ReportRecord(BaseModel):
    ...
    artifact_keys: List[str] = Field(
        default_factory=list, description="Optional artifact keys"
    )
...
class EvaluateDecision(BaseModel):
    task_id: str = Field(..., description="ID of the task")
    action: Literal["mark_complete", "retry", "escalate"] = Field(
...
class FinalResult(BaseModel):
    ...
    supporting_context: List[str] = Field(
        default_factory=list, description="Supporting context entries"
    )
