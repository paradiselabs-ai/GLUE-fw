...
class DynamicComponentStore:
    ...
    def _ensure_storage_dir(self) -> None:
        ...
        for component_type in ComponentType:
            os.makedirs(
                os.path.join(self.storage_dir, component_type.value), exist_ok=True
            )

    def _get_component_path(
        self, component_id: str, component_type: Optional[ComponentType] = None
    ) -> str:
        """
        ...
        if component_type:
            return os.path.join(
                self.storage_dir, component_type.value, f"{component_id}.json"
            )

        # Search in all component type directories
        for type_value in ComponentType:
            path = os.path.join(
                self.storage_dir, type_value.value, f"{component_id}.json"
            )
    ...
    async def _save_spec_to_file(self, spec: ComponentSpec) -> None:
        ...
        os.makedirs(
            os.path.join(self.storage_dir, spec.component_type.value), exist_ok=True
        )
