...
class AnthropicProvider:
    ...
    def _initialize_client(self):
        ...
        try:
            ...
            self.client = type(
                "MockClient",
                (),
                {"messages": type("MockMessages", (), {"create": None})},
            )
    ...
    def _get_api_key(self) -> str:
        ...
        if hasattr(self.model, "development") and self.model.development:
            logger.warning(
                "No Anthropic API key provided but running in development mode. Using dummy key."
            )
    ...
    def _load_api_key_from_dotenv(self) -> Optional[str]:
        ...
        try:
            ...
            logger.warning(
                "python-dotenv package not installed. Unable to load from .env file."
            )
            return None
        except Exception as e:
    ...
    async def generate_response(
        ...
        try:
            ...
            if (
                ...
                and any(c.type == "tool_use" for c in response.content)
            ):
