...
import os
import logging
from typing import Dict, List, Any, Optional, Callable, AsyncIterable, Type

from glue.core.schemas import Message, ToolCall, ToolResult
from glue.utils.portkey_client import PortkeyClient, PortkeyConfig, get_portkey_client
    ...
    async def generate(
        ...
        **kwargs
        ...
        if self.portkey_provider == "openai":
            ...
            logger.warning(f"Portkey integration for {self.portkey_provider} not fully implemented")
            return await self.provider.generate(messages, tools, **kwargs)
        
        try:
            # Make the request through Portkey
            response = await self.portkey_client.proxy_request(
                self.portkey_provider, 
                endpoint, 
                payload
            )
            return response
