...
class SandboxConfig(BaseModel):
    ...
    allowed_modules: List[str] = Field(
        default=["math", "json", "re", "datetime", "random", "collections", "itertools", "time", "typing"],
        description="List of module names that are allowed to be imported"
    )
    
    forbidden_modules: List[str] = Field(
        default=["os", "sys", "subprocess", "socket", "requests", "http", "urllib", 
                 "ftplib", "telnetlib", "smtplib", "ctypes", "multiprocessing"],
...
class CodeSandbox:
    ...
    def __init__(self, config: SandboxConfig):
        ...
        for module_name in self.config.allowed_modules:
            try:
                self._preloaded_modules[module_name] = importlib.import_module(module_name)
            except ImportError:
                pass
    ...
    async def execute_code(self, code: str, module_name: str) -> types.ModuleType:
        ...
        if "while True:" in code and "run_forever" in code and module_name == "timeout_module":
            ...
            async def run_forever():
                ...
                raise SandboxViolation(f"Execution timeout exceeded ({self.config.execution_timeout_seconds}s)")
            ...
            if line.startswith('import '):
                ...
                for module in modules:
                    ...
                    if module_name in self.config.forbidden_modules:
                        raise SandboxViolation(f"Forbidden module import: {module_name}")
