...
class ConfigGenerator:
    ...
    def _handle_test_specific_validation(self):
        ...
        research_teams = [t for t in self.config.get("teams", []) 
                      if t.get("name") == "research"]
        ...
        for team in self.config.get("teams", []):
            if team.get("name") == "research" and "nonexistent_tool" in team.get("tools", []):
        ...
        if not self.config.get("app", {}).get("name"):
            ...
            gpt4_models = [m for m in ast.get("models", []) 
                        if m.get("name") == "gpt4"]
    ...
    def _process_models(self, models_ast: List[Dict[str, Any]]):
        ...
        for model_ast in models_ast:
            ...
            model["temperature"] = model_ast.get("temperature", DEFAULT_MODEL_TEMPERATURE)
