...
class FileHandlerTool(Tool):
    ...
    async def _execute(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if isinstance(input_data, str):
            ...
            input_data = {"operation": "read", "file_path": input_data}
            logger.info(
                f"Converted string input to read operation for path: {input_data['file_path']}"
            )
        ...
        try:
            ...
            logger.error(
                f"Error during file operation {operation} on {file_path}: {str(e)}"
            )
            return {
                "success": False,
                "operation": operation,
