...
FENCED_JSON_REGEX = re.compile(r"```json\s*(.*?)\s*```", re.DOTALL | re.IGNORECASE)
...
def normalize_glue_tool_call(
    ...
    if (
        ...
        if tool_name not in VALID_TOOL_NAMES:
            logger.error(
                f"Invalid tool name: {tool_name}. Expected one of {VALID_TOOL_NAMES}"
            )
...
def attempt_json_recovery(json_str: str) -> Optional[str]:
    ...
    with log_context("attempt_json_recovery"):
        ...
        try:
            json.loads(json_str)
            logger.debug("Recovered JSON after cleanup")
            return json_str
        except json.JSONDecodeError as e:
        ...
        try:
            json.loads(json_str)
            logger.debug("Recovered JSON after typo fixes")
            return json_str
        except json.JSONDecodeError as e:
...
def extract_json(
    ...
    with log_context("extract_json"):
        ...
        # Extract JSON string
        json_str = extract_json_from_fenced(cleaned_text) or extract_json_from_bare(
        ...
        def parse_json(s: str) -> Optional[Dict[str, Any]]:
            try:
                ...
                if normalized and ("tool_name" in normalized or "tool_name" in data):
                    return (
                        normalized
                        if glue_compatibility
                        else {"data": normalized, "tool_call": True}
                    )
                    ...
                    return (
                        data
                        if glue_compatibility
                        else {"data": data, "tool_call": False}
                    )
            except json.JSONDecodeError as e:
