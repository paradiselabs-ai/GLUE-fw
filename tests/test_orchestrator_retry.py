...
from glue.core.types import TaskStatus, DEFAULT_MAX_RETRIES
...
def dummy_team():
    ...
    return SimpleNamespace(config=SimpleNamespace(lead='agent1', members=['agent1', 'agent2']), name='team')

@pytest.fixture
def loop(dummy_team):
    # Create orchestrator with no LLM (fallback)
    return TeamLeadAgentLoop(team=dummy_team, delegate_tool=DummyDelegateTool(), agent_llm=None)

@pytest.mark.asyncio
async def test_retry_logic_less_than_max(loop):
    # Setup single subtask with retries < max
    record = TaskRecord(description='desc', dependencies=[], state=TaskStatus.PENDING.value, retries=0)
    record.task_id = 'task-1'
    loop.task_states = {'sub1': record}
    report = ReportRecord(task_id='task-1', status='failure', detailed_answer='error', artifact_keys=[], failure_reason=None)
...
async def test_retry_logic_exceeds_max(loop):
    ...
    loop.max_retries = 1
    record = TaskRecord(description='desc', dependencies=[], state=TaskStatus.PENDING.value, retries=1)
    record.task_id = 'task-2'
    loop.task_states = {'sub2': record}
    report = ReportRecord(task_id='task-2', status='failure', detailed_answer='error', artifact_keys=[], failure_reason=None)
    await loop._evaluate_report(report)
