"""
Tests for the Anthropic provider implementation in the GLUE framework.

This module contains tests for the Anthropic provider class that handles
communication with the Anthropic API for model interactions.
"""
import pytest
import os
...
# Mock the anthropic module since we don't have it installed
@pytest.fixture(autouse=True)
def mock_anthropic_import():
    """Mock the anthropic import to avoid ImportError"""
...
class TestAnthropicProvider:
    ...
    def setup_method(self):
        ...
        self.config = ModelConfig(
            ...
            temperature=0.7,
            max_tokens=2048,
            description="Helpful assistant that can use tools."
        )
        
        # Mock the _initialize_client method to prevent actual API client creation
        with patch.object(AnthropicProvider, '_initialize_client'):
            self.model = BaseModel(self.config)
            self.provider = AnthropicProvider(self.model)
    ...
    async def test_generate_response_basic(self):
        ...
        self.provider.client.messages.create = AsyncMock(return_value=mock_response)
        ...
        self.provider.client.messages.create.assert_called_once()
        
        # Verify the call arguments
        call_args = self.provider.client.messages.create.call_args[1]
    ...
    async def test_generate_response_with_tools(self):
        ...
        mock_tool_call.input = {"query": "test query"}
    ...
    async def test_api_key_from_environment(self):
        ...
        try:
            ...
            with patch('anthropic.Anthropic', return_value=mock_anthropic) as mock_anthropic_class:
                config = ModelConfig(
                    name="assistant",
                    provider=ModelProvider.ANTHROPIC,
                    model="claude-3-opus-20240229",
                ...
                with patch.object(BaseModel, '_initialize_client'):
                    ...
                    with patch.object(AnthropicProvider, '_initialize_client') as mock_init:
    ...
    async def test_api_key_from_dotenv(self):
        ...
        try:
            ...
            with open(".env.test", "w") as f:
                f.write("ANTHROPIC_API_KEY=dotenv_test_key\n")
                ...
                with patch('os.path.exists', return_value=True):
                    ...
                    with patch.dict('os.environ', {}, clear=True):
                        ...
                        def mock_load_dotenv_effect(*args, **kwargs):
                            os.environ["ANTHROPIC_API_KEY"] = "dotenv_test_key"
                            return True
